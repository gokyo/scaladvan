 ...
 * Note:
 *  
 * Running this Scala application from Eclipse, it doesn't show you the Wikipedia suggestions.
 * Running it from SBT, it works.
 */
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val selections: Observable[String] =
      (button clicks) filter {
        (_: Button) => !suggestionList.selection.items.isEmpty
      } map {
        (_: Button) => suggestionList.selection.items(0)
      }
