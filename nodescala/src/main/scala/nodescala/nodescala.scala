...
trait NodeScala {
   ...
   *
   *  @param exc          the exchange used to write the response back
   *  @param token        the cancellation token for
   *  @param res          the response to write back
   */
  private def respond(exc: Exchange, token: CancellationToken, res: Response): Unit = {
    
    res takeWhile {
      _: String => token.nonCancelled
    } foreach {
      r: String => exc.write(r)
    }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val startSub: Subscription = listener.start
    
    val cancelSub: Subscription = Future.run() {
      
      (ct: CancellationToken) =>
        
        (async {
          while (ct.nonCancelled) {
            // Awaits next request from listener
            val (req, exc) = await { listener.nextRequest }
            // Answers asynchronously
            Future { respond(exc, ct, handler(req)) }
          }
...
object NodeScala {
  ...
  /**
   * A request is a multi-map of headers, where each header is a key-value pair of strings.
   */
  ...
  trait Listener {
     ...
     *     and then de-registers itself using `removeContext`
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      this.createContext {
        (handler: Exchange) => p.success((handler.request, handler))
        this.removeContext
      }
      p.future
    }
