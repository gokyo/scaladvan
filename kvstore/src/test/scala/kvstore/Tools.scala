...
trait Tools { 
  ...
  this: TestKit with FunSuite with ShouldMatchers with ImplicitSender =>
  
  import Arbiter._
  import Tools._

  def probeProps(probe: TestProbe): Props =
    Props(classOf[TestRefWrappingActor], probe)
  ...
  class Session(val probe: TestProbe, val replica: ActorRef) {
    ...
    @volatile
    private var referenceMap: Map[String, String] =
    ...
    def setAcked(key: String, value: String): Unit =
      waitAck(set(key, value))
    ...
    def nothingHappens(duration: FiniteDuration): Unit = 
      probe.expectNoMsg(duration)
  }

  def session(replica: ActorRef)(implicit system: ActorSystem) = 
    new Session(TestProbe(), replica)
